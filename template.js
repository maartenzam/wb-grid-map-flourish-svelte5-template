  ...
  function WorldHexGrid($$anchor, $$props) {
    ...
    function generateHexLayout(grid, size2) {
      let hexGrid2 = grid.map((d) => {
        let hex2 = {};
        let x = size2 + d.q * 3 / 2 * size2;
        let y = equals(d.q % 2, 0) ? 2 * size2 + d.r * size2 * 2 * shift2 : 2 * size2 + d.r * size2 * 2 * shift2 - Math.sqrt(3) * size2 / 2;
        hex2.q = d.q;
        hex2.r = d.r;
        hex2.iso3c = d.iso3c;
        hex2.x = x;
        ...
        for (let i = 0; i < 6; i++) {
          ...
          vertices.push([
            x + size2 * Math.cos(angle_rad),
            y + size2 * Math.sin(angle_rad)
          ]);
        }
